...
private:
	...
	//
	formatting_options optOld;
	//
	formatter fmt;
	std::string text;
	//
	std::string textinfont;
public:
	...
	{
		std::string formattext = fmt.format(opt);
		//
		if (formattext.empty())
		{
			textinfont += s;
	...
	//
	void AppendTextInFont()
	{
		if (textinfont.empty())
			return;
		//
		int nCodePage = optOld.chpFont.GetCodePage();
		//
		CStringW strTextW = WizSTR2BSTR(nCodePage, textinfont.c_str());
		//
		CStringA strTextA = WizBSTR2STR(CP_UTF8, strTextW);
		//
		text += strTextA;
		//
		textinfont.clear();
...
public:
	...
	template <class T> void writeHiddenText(T s)
	{
		text+=s;
	}
